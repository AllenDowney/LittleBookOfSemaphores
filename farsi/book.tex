...
\begin{lstlisting}[title=\rl{راه حل بار سوشی  \#2}h]{}
mutex.wait()
if must_wait:
    waiting += 1
    mutex.signal()
...
%mutex.  The fifth customer sets {\tt must\_wait}.
    ...
    پنجمین مشتری مقدار {\tt must\_wait} را برابر \texttt{True} می‌کند. 
...
%to us to get it right.
    اگر {\tt must\_wait} مقدار صحیح داشته باشد تا زمانیکه آخرین مشتری مقدار آن را ناصحیح نموده و به  {\tt block} سیگنال دهد مشتری‌های جدید 
    مسدود می‌شوند. قابل درک است که نخ سیگنال دهنده از میوتکس صرف نظر نموده و نخ منتظر آن را می‌گیرد. 
