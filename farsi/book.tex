...
% with others.
در این فصل تعدادی از مسائل همگام سازی پایه ارائه شده است و نشان داده می شود چگونه با استفاده از سمافورها آن ها را حل کنیم. این مسائل شامل تسلسل و موچوال اکسکلوژن - که قبلا با آن ها آشنا شده ایم - به همراه سایر موضوعات می شوند.
...
احتمالا علامت دهی ساده ترین استفاده از یک سمافور است، و به این معناست که یک نخ پیامی به نخ دیگر می فرستد تا اعلام کند چیزی اتفاق افتاده است.
...
% problem.
با علامت دهی این امکان فراهم می شود تا مطمئن شویم که یک قطعه کد از یک نخ حتما قبل از قطعه کدی دیگر در نخی دیگر اجرا خواهد شد؛ به عبارت دیگر، مسئله ی تسلسل را حل می کند.

% Assume that we have a semaphore named {\tt sem} with initial value
% 0, and that Threads A and B have shared access to it.
فرض کنید یک سمافور با نام sem و مقدار اولیه ی 0 داریم، و نیز فرض کنید نخ های A و B هر دو به آن دسترسی دارند.
...
کلمه ی statement نشان دهنده ی یک عبارت دلخواه در برنامه است. برای اینکه مثال مشخص تر شود، فرض کنید a1 یک خط از یک فایل را می خواند، و b1 آن خط را در صفحه نمایش نشان می دهد. سمافور در این برنامه تضمین می کند که نخ A عملیات a1 را، قبل از آنکه نخ B عملیات b1 را شروع کند، به طور کامل انجام داده است.
...
% (little hint there).
می خواهیم مطمئن شویم که a1 پیش از b2 رخ می دهد و نیز b1 قبل از a2 اتفاق می افتد. هنگام نوشتن راه حل خود، نام و مقدار اولیه ی سمافورها را حتما مشخص نمایید (اشاره کوچکی وجود دارد).
