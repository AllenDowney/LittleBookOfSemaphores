...
%can proceed, and never will.
    ...
    در خط \texttt{wait} مسدود می‌شود. وقتیکه \lr{B}  برسد، آن نیز مسدود خواهد شد زیرا که \lr{A} نمی‌تواند به \lr{\texttt{aArrived}} 
...
%enforce mutual exclusion to the shared variable {\tt count}.
    معمّا: سمافورهایی به مثال زیر بیفزایید تا انحصار متقابل متغیر اشتراکی \lr{\texttt{count}} را اعمال نمایند. 
...
%arrive will have to wait until the first signals.
    از آنجایی که مقدار اولیه  \lr{mutex}  $1$ است، اولین نخی که \texttt{wait} در کد خود می‌رسد می‌تواند بلافاصله ادامه یابد.
    البته عمل انتظار روی سمافور موجب کاهش مقدار آن می‌گردد لذا دومین نخ به  \texttt{wait} می‌رسد باید منتظر پیام‌دهی نخ اول بماند. 
...
%at a time, and it can be locked to bar all threads.
    این الگو --یک \texttt{wait} و یک \texttt{signal} بلافاصله پشت سر هم-- آنقدر رایج است که یک نام دارد: \lr{turnstile}
...
%to invoke {\tt wait} before the Barrier opens.
    ...
    پارامتر \lr{\texttt{n}} تعداد نخ‌هایی است که باید پیش از اینکه حصار باز شود \texttt{wait}  را فراخوانند. 
...
\clearemptydoublepage
\subsection {راهنمایی برای معمّا}
